...
b = as.vector(rbind(b.1,b.2,b.3))
Y = scale(pinv(A)%*%b + (diag(rep(1,n))-pinv(A)%*%A)%*%h,scale=F)
# var((diag(rep(1,n))-p.A%*%A)%*%h)
# sqrt(t(Y - X%*%solve(t(X)%*%X)%*%t(X)%*%Y) %*% (Y - X%*%solve(t(X)%*%X)%*%t(X)%*%Y)/(n-p-1) * solve(t(X)%*%X)[1,1])

lm1 <- summary(lm(Y~X))
lm2 <- summary(lm(Y~X[,1]))
lm3 <- summary(lm(Y~X[,2]))
lm1.std.error <- lm1$coefficients[,2][-1]
lm2.std.error <- lm2$coefficients[,2][-1]
lm3.std.error <- lm3$coefficients[,2][-1]
